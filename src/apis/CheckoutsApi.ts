...
export class CheckoutsApi extends runtime.BaseAPI {
     ...
     * List Checkout Sessions
     */
    async listCheckoutSessions(merchantId: string, limit?: number, offset?: number, filter?: string, sort?: string, order?: string, status?: string, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<ListCheckoutSessionsResponseDto> {
        const response = await this.listCheckoutSessionsRaw({ merchantId: merchantId, limit: limit, offset: offset, filter: filter, sort: sort, order: order, status: status }, initOverrides);
        return await response.value();
    }
